...
public class MyFixApplicationDataDictionary implements Application {
    ...
    public void fromApp(Message message, SessionID sessionId)
        ...
        String ordType = message.getString(quickfix.field.OrdType.FIELD);
    ...
    private NewOrderSingle createNewOrderSingleMessage() {
        ...
        newOrderSingle.setChar(quickfix.field.OrdType.FIELD, '1');
        newOrderSingle.setString(quickfix.field.Side.FIELD, "1");
        newOrderSingle.setString(quickfix.field.Symbol.FIELD, "1");
        newOrderSingle.setString(quickfix.field.Text.FIELD, "a");
    ...
    public static void main(String[] args) throws FieldConvertError {
        ...
        // Acceptor (サーバー) を起動
        try {
            SessionSettings settings = new SessionSettings("acceptor_datadictionary.cfg");
            MessageStoreFactory storeFactory = new FileStoreFactory(settings);
            LogFactory logFactory = new FileLogFactory(settings);
            MessageFactory messageFactory = new DefaultMessageFactory();
            Acceptor acceptor = new SocketAcceptor(new MyFixApplicationDataDictionary(), storeFactory, settings, logFactory, messageFactory);
            acceptor.start();
        ...
        try {
            SessionSettings settings = new SessionSettings("initiator_datadictionary.cfg");
            MessageStoreFactory storeFactory = new FileStoreFactory(settings);
            LogFactory logFactory = new FileLogFactory(settings);
            MessageFactory messageFactory = new DefaultMessageFactory();
            Initiator initiator = new SocketInitiator(new MyFixApplicationDataDictionary(), storeFactory, settings, logFactory, messageFactory);
            initiator.start();
        } catch (ConfigError | RuntimeError e) {
            e.printStackTrace();
        }
